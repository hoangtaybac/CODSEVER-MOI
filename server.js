...
// ✅ Chuẩn hóa ký hiệu độ trong LaTeX: 30 \circ / 30° / 30 ^ {\circ} -> 30^{\circ}
function normalizeDegreeLatex(latex) {
  if (!latex) return latex;
  let s = String(latex);

  // tránh đổi lại những công thức đã đúng sẵn
  s = s.replace(/([0-9]+(?:[.,][0-9]+)?|[A-Za-z])\s*\^\s*\{\s*\\circ\s*\}/g, "$1^{\\circ}");

  // 30 \circ hoặc 45.5 \circ hoặc x \circ
  s = s.replace(/([0-9]+(?:[.,][0-9]+)?|[A-Za-z])\s*\\circ\b/g, "$1^{\\circ}");

  // 30° hoặc 45,5° hoặc x°
  s = s.replace(/([0-9]+(?:[.,][0-9]+)?|[A-Za-z])\s*°/g, "$1^{\\circ}");

  // Các lỗi OCR hay gặp: 30 ^ \circ, 30 ^{ circ }
  s = s.replace(/([0-9]+(?:[.,][0-9]+)?|[A-Za-z])\s*\^\s*\\circ\b/g, "$1^{\\circ}");
  s = s.replace(/([0-9]+(?:[.,][0-9]+)?|[A-Za-z])\s*\^\s*\{\s*circ\s*\}/gi, "$1^{\\circ}");
...
app.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    normalizeLatexMapDegrees(latexMap);
